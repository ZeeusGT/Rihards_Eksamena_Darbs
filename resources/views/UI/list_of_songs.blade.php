...
<head>
    ...
    <link rel="stylesheet" href="/public/song_list_dependencies/styles/song_list_styles.css">
    <script src="/public/song_list_dependencies/js/song_list_scripts.js"></script>
</head>
<body>
    ...
    @if (count($results) > 0)
        @foreach ($results as $result)
            ...
            @else
                <tr class="TableOdd">
            @endif
                <td onclick="likeBeforeRedirect('{{ route('songs.view', ['song' => $result->id]) }}')">{{ $result->Song_Name }}</td>
                <td onclick="likeBeforeRedirect('{{ route('songs.view', ['song' => $result->id]) }}')">{{ $result->Artists_Name }}</td>
                <td onclick="likeBeforeRedirect('{{ route('songs.view', ['song' => $result->id]) }}')">{{ $result->Songs_Genre }}</td>
                <td>
                    <div class="Action_Container">
                        <label>
                            <input type="checkbox" name="liked_songs[]" id="check{{ $result->id }}"
                            {{ in_array($result->id, Auth::user()->Liked_Songs) ? 'checked' : '' }}
                            onchange="likefunc('{{ $result->id }}')">
                            <span><i class="fa-regular fa-heart"></i></span>
                        </label>
                        <audio class="AudioClass" id="Song{{ $result->id }}" src="/public/Songs_Folder/{{ $result->Files_Name }}"></audio>
                        <div class="PlayButton" id="PlayButtonWithId{{ $result->id }}" onclick="Play({{ $result->id }})">
